 ...
 */
#ifndef __LIB_SPREGION__
#define __LIB_SPREGION__
...
class libSPRegion : public libSPBase<T, DHB> {
public:
...
private:
	...
	T ArmijoStep(std::vector<T> *theta, std::vector<T> *empMean, std::vector<T>& grad);
protected:
	int SPClear();
public:
	libSPRegion(typename DHB::SPParameters* params);
	~libSPRegion();
