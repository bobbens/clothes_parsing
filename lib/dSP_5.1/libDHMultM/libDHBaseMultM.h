 ...
 */
#ifndef __LIB_DHBASEMULTM__
#define __LIB_DHBASEMULTM__

#include <vector>
#include <map>

template <class T, class DHB> class libSPRegionMultM;

template <class T>
class libDHBaseMultM {
	friend class libSPRegionMultM<T, libDHBaseMultM<T> >;
private:
	...
	virtual int ExchangeNu() = 0;
	virtual int ExchangeNuPerSample() = 0;
protected:
...
public:
	...
	virtual ~libDHBaseMultM() {};

	struct SPParameters {
		T epsilon;
		T p;
		T C;
		size_t CRFOuterIterations;
		...
		SPParameters() : epsilon(T(0.0)), p(T(2.0)), C(T(1.0)), CRFOuterIterations(1), CRFMPIterations(1), ReuseMessagesForF(1), argc(0), argv(NULL), CRFIterations(10), ArmijoIterations(50), CRFEraseMessages(0), CRFOuterExchange(size_t(-1)), MPIterations(50), ReadBinary(0), Verbosity(0) {;}
	};
};
